...
#define PN532_FRAME_MAX_LENGTH              30
#define PN532_DEFAULT_TIMEOUT               1000


extern unsigned int tick ;
extern unsigned int timePassive,timeAuth,timeRead,time1,time2,time3,time4,time5,tickTime,timeWrite;
...
//		time1 = HAL_GetTick() - tickTime;
//		
//		tickTime = HAL_GetTick();		
    if (!pn532->wait_ready(timeout)) {
        return PN532_STATUS_ERROR;
    }
		//time2 = HAL_GetTick() - tickTime;
...
/**
  ...
  */
int PN532_GetFirmwareVersion(PN532* pn532, uint8_t* version, uint8_t sendVersion) {
    // length of version: 4
    if (PN532_CallFunction(pn532, PN532_COMMAND_GETFIRMWAREVERSION,
        ...
        //pn532->log("Failed to detect the PN532");
				if(sendVersion)										 
					printf("{\"C\":{\"ReaderType\":PN532,\"Version\":Error}}\n");	
        return PN532_STATUS_ERROR;
    }
		if(sendVersion)											 
			printf("{\"C\":{\"ReaderType\":PN532,\"Version\":%d.%d.%d.%d}}\n",version[0],version[1],version[2],version[3]);	
    return PN532_STATUS_OK;
}
...
int PN532_ReadPassiveTarget(
    ...
    int length = PN532_CallFunction(pn532, PN532_COMMAND_INLISTPASSIVETARGET,
		...
		//printf("Time Pasive = %u\n", HAL_GetTick() - li);
		//timePassive = HAL_GetTick() - tick;
    if (length < 0) {
...
//		timeAuth = HAL_GetTick() - tick;
//		printf("Authenticate = %u\n", timeAuth);
    return response[0];
}
