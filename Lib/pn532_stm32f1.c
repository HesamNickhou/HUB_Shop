...
void i2c_read(uint8_t* data, uint16_t count) {
		i2cStatus = HAL_I2C_Master_Receive(&hi2c1, _I2C_ADDRESS, data, count, _I2C_TIMEOUT);
}
//*******
void i2c_write(uint8_t* data, uint16_t count) {
    i2cStatus = HAL_I2C_Master_Transmit(&hi2c1, _I2C_ADDRESS, data, count, _I2C_TIMEOUT);
}
//**********
