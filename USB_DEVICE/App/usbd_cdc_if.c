...
extern unsigned char serialConnect,serialConnectWithTab;
extern unsigned char fromAndroid[600];
extern unsigned int CDCReceiveLen;
#ifdef MaxReciveBuf
#define HL_RX_BUFFER_SIZE 600 // Can be larger if desired	
//extern volatile uint8_t rxBuffer[HL_RX_BUFFER_SIZE]; // Receive buffer
extern  uint16_t rxBufferHeadPos; // Receive buffer write position
extern  uint16_t rxBufferTailPos; // Receive buffer read position
#endif
...
//	 memset(fromAndroid,'\0',sizeof(fromAndroid));
	/*
	#if !defined(Printer) && !defined(finger) 
	for(uint32_t i=0;i<CDCReceiveLen;i++)
		fromAndroid[i] = Buf[i];
	#endif
	*/	
  USBD_CDC_SetRxBuffer(&hUsbDeviceFS, &Buf[0]);
//  USBD_CDC_ReceivePacket(&hUsbDeviceFS);//sk W
											#ifdef MaxReciveBuf
...
USBD_CDC_ReceivePacket(&hUsbDeviceFS);//sk C
...
//**************************
	void CDC_FlushRxBuffer_FS() {
		for (unsigned int i = 0; i < HL_RX_BUFFER_SIZE; i++)
			fromAndroid[i] = 0;
		  rxBufferHeadPos = 0;
		  rxBufferTailPos = 0;
	}
//***************
	uint8_t CDC_ReadRxBuffer_FS(uint8_t* Buf, uint16_t Len)
	{
		...
		for (uint8_t i = 0; i < Len; i++) {
			Buf[i] = fromAndroid[rxBufferTailPos];
